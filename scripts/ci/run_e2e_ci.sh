...
PERF_GATEWAY_BASE_URL="${PERF_GATEWAY_BASE_URL:-http://127.0.0.1:8080}"
PERF_NOTIFY_WS_BASE_URL="${PERF_NOTIFY_WS_BASE_URL:-ws://127.0.0.1:8090/ws}"
PERF_NOTIFY_HTTP_BASE_URL="${PERF_NOTIFY_HTTP_BASE_URL:-http://127.0.0.1:8090}"
...
main() {
  ...
  if [[ "${RUN_PERF_BASELINE}" == "1" ]]; then
    ...
    if ! PERF_LOG_DIR="${CI_LOG_DIR}/perf" \
      GATEWAY_BASE_URL="${PERF_GATEWAY_BASE_URL}" \
      NOTIFY_WS_BASE_URL="${PERF_NOTIFY_WS_BASE_URL}" \
