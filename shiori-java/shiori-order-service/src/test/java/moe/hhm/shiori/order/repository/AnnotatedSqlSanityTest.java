...
class AnnotatedSqlSanityTest {
    ...
    void shouldNotLeaveXmlCDataMarkersInSelectAnnotations() {
        assertNoXmlCDataMarkers(OrderMapper.class, OrderReviewMapper.class);
    }

    private void assertNoXmlCDataMarkers(Class<?>... mapperTypes) {
        List<String> invalidMethods = Arrays.stream(mapperTypes)
                .flatMap(mapperType -> Arrays.stream(mapperType.getDeclaredMethods()))
